...
__global__ void incrementTime(int* lattice){
    ...
    __global__ void initialize(dcmplx *QField, int* lattice, int* gpu_params){
        ...
        dcmplx i(0.,1.);
        int vectorSize = 2;
        double numGaussPairs = 2.0;
        double norm = (1./numGaussPairs);
        
        double sigma0 = 0.05*(double)xSize;
        double shift0 = 0.35*(double)xSize;
        double p0 = 0.0;
        dcmplx phaseKick0 = exp(i*(-p0*2.*pi)*(this_x)/(double)(xSize));
        double sigma1 = 0.05*(double)xSize;
        double shift1 = 0.65*(double)xSize;
        double p1 = 0.0;
        dcmplx phaseKick1 = exp(i*(-p1*2.*pi)*(this_x)/(double)(xSize));
        int n;
        dcmplx field = norm*(Mul(phaseKick0, gaussian(this_x,sigma0,shift0))+Mul(phaseKick1, gaussian(this_x,sigma1,shift1)));
