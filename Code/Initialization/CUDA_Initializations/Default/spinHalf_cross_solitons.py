def get_CUDA(vectorSize = 4, G0 = 1.5, G1 = -0.5, MU = -1., scaling = 1., momentums = [0.,0.], positions = [0.5,0.5], dim2=0, coeffs=[1.,1.]):
  pos_mom_string = '''dcmplx Cm(''' + str(coeffs[0].real) + ''',''' + str(coeffs[0].imag) + r''');'''+'''
      dcmplx Cp(''' + str(coeffs[1].real) + ''',''' + str(coeffs[1].imag) + r''');
      '''
 ...
 __global__ void initialize(dcmplx *QField, int* lattice, int* gpu_params){
  ...
  dcmplx fieldM = norm*(0.5*k*Cm/cosh(k*A*(this_x-cen0))*phase1+0.5*''' + str(dim2) + r'''*k*Cm/cosh(k*A*(this_y-cen1))*phase2);
  dcmplx fieldP = norm*(0.5*k*Cp/cosh(k*A*(this_x-cen0))*phase1+0.5*''' + str(dim2) + r'''*k*Cp/cosh(k*A*(this_y-cen1))*phase2);
