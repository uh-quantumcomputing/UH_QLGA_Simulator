...
def get_states(g0, g1, g2, state_num, c1 = 1./2., c2 = 1./2., **kwargs):
	...
	if state_num == 1:
		if g0 + 4*g1 < 0:
			print ("Critical Error: incompatible g's and kink state")
			return "ERROR"
		else:  
	...
	### STATE 11 ###
	if state_num == 11:
		if (-2*c2*c2*(5.*g0 + g2) + 10)/(5.*g0 + g2) < 0:
			print ("Critical Error: incompatible c2 and g's and kink state")
			return "ERROR"
		else:  
			return [1. ,0.,  (10-2*c2*c2*(5.*g0 + g2))/(5.*g0 + g2), 0.,1., c2,1.,1.,1.,c2]
	### STATE 12 ###
	if state_num == 12:
	...
	### STATE 16 ###
	if state_num == 16:
		if (10. - ((2.*c1*c1) + c2*c2)*(5.*g0 + g2))/(10.*g0 + 2.*g2) < 0:
			print ("Critical Error: incompatible g's and kink state")
			return "ERROR"
		else:  
			return [1.,1., (10. - 2.*(c2*c2 + c1*c1)*(5.*g0 + g2))/(5*g0 + g2), 1., 1., c2,c1,1.,-c1, c2]
	### STATE TOO HIGH ####
	if state_num >= 17:
		print ("Critical Error: no kink state with number > 16")
		return "ERROR"
