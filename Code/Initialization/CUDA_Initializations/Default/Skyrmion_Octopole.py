            ...
            for (int q = -1; q <2 ; q +=2){
              for (int p = -1; p <2 ; p +=2){
                for (int t = -1; t <2 ; t +=2){
                  ...
                  double z_center_skyrmion = z_center_new + double(t)*separation;
                  double r_squared = get_r_squared_3d(this_x, this_y, this_z, x_center_skyrmion, y_center_skyrmion, z_center_skyrmion, AA); 
                  double r = sqrt(r_squared);
                  dcmplx field = 1.;
                  if (r>0){
                    field = cos(__dmul_rn(pi,tanh(__dmul_rn(K,r))))-Mul3(ii,sin(__dmul_rn(pi,tanh(__dmul_rn(K,r)))),__ddiv_rn(__dmul_rn(t*A,(this_z-z_center_skyrmion)),r));
                  }
