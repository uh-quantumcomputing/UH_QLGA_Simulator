...
def get_CUDA(vectorSize = 10, quad_distance = 1./4., separation = 1./4., G0 = -.7, G1 = .5, G2 = .6, MU = 1., scaling = 30, 
            first_solution = 1, second_solution = 0, p1x = 0., p1y = 0.):
  a1, a2, b1 = pade.get_pade_quad(G0, G1, G2, MU, first_solution)
  A1, A2, B1 = pade.get_pade_quad(G0, G1, G2, MU, second_solution)
  mf_array = np.asarray(pade.get_mfs(first_solution), dtype = np.int_)
  mf_array2 = np.asarray(pade.get_mfs(second_solution), dtype = np.int_)
  return r'''
 __global__ void initialize(dcmplx *QField, int* lattice, int* gpu_params){
  ...
  int stream_distance = __double2int_rn(__dmul_rn(''' + str(quad_distance) + r''', ySize));
  ...
  a2 = ''' + str(A2) + r''';
  b1 = ''' + str(B1) + r''';
  int is_mf_empty2 [5] = {''' + str(int(mf_array2[0])) + r''',''' + str(int(mf_array2[1])) + r''', ''' + str(int(mf_array2[2])) + r''', ''' + str(int(mf_array2[3])) + r''', ''' + str(int(mf_array2[4])) + r'''};
  for (int mf = 0; mf < spinComps; mf ++){
    dcmplx result(1., 0.);
    if (is_mf_empty2[mf] > 0) {
      for (int i = 0; i < square_size; i ++){
        for (int j = 0; j < square_size; j ++){
          int adjusted_i = i - square_size/2;
          int adjusted_j = j - square_size/2;
        ...
        dcmplx phase_velocity = Mul(phase_velocity1, phase_velocity2);
        result = Mul(phase_velocity, result);
        QField[2*mf+z*vectorSize+((y+stream_distance)%ySize)*vectorSize*zSize+x*zSize*ySize*vectorSize] = Mul3(half, (dcmplx) is_mf_empty2[mf], result);
        QField[2*mf+1+z*vectorSize+((y+stream_distance)%ySize)*vectorSize*zSize+x*zSize*ySize*vectorSize] = Mul3(half, (dcmplx) is_mf_empty2[mf], result);
      }
    }
}'''