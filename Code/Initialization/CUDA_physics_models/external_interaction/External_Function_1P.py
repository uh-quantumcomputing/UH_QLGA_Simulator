...
def get_CUDA(dimensions, vectorSize, cond_list=['true'] ,func_list = ['sin(X)'], **kwargs):
	...
	int time_step = lattice[14]; \\Number of applications of full evolution operator
	int deviceNum = gpu_params[0]; \\Index of GPU device this compute thread is on
	int numGPUs = gpu_params[2]; 
	int xSize = lattice[0]*numGPUs;
	int ySize = lattice[2];
	int zSize = lattice[4];
	int vectorSize = ''' + str(vectorSize) + '''; \\ Dimension of modeled wavefunction base on model
	...
	\\ Put in CUDA code generated by user input
	''' + potentialString + '''
