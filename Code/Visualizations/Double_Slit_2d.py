...
def animate(i, fig, file_name):
    ...
    RhoFieldComp = np.zeros((xSize, ySize, vectorSize), dtype = DTYPE)
    ...
    gpuRhoFieldComp = drv.to_device(RhoFieldComp)
    gpuLattice = drv.to_device(Lattice)
    getPlotDetails(gpuQField, gpuVField, gpuRhoField, gpuPhaseField, gpuVxField, gpuVyField, gpuRhoFieldComp, gpuLattice, 
    ...
    RhoFieldComp = drv.from_device(gpuRhoFieldComp, RhoFieldComp.shape, DTYPE)
    ...
    # Plot mf = 0 rho
    rho_mf0 = (RhoFieldComp[:,:,4]+RhoFieldComp[:,:,5]).real
    rho_mf0_max = np.amax(rho_mf0)
    plt.subplot(211)
    plt.imshow((rho_mf0), extent=(np.amin(yAxis), np.amax(yAxis), np.amin(xAxis), np.amax(xAxis)), origin = 'lower',
        alpha = 0.6, cmap=blues_alpha, norm=colors.SymLogNorm(linthresh=linThresh*rho_mf0_max,linscale=linScale,vmin=0.,vmax=rho_mf0_max))#vmax=rhoMax))
    putLabels('', r'$x\ \ (\ell)$', r'$\rho \ \ (\frac{1}{\ell^2})$')
    ax = plt.gca()
    ax.set_aspect('auto')

    # Plot mf = -1 rho
    rho_mf1 = (RhoFieldComp[:,:,2]+RhoFieldComp[:,:,1]).real
    rho_mf1_max = np.amax(rho_mf1)
    plt.subplot(211)
    plt.imshow((rho_mf1), extent=(np.amin(yAxis), np.amax(yAxis), np.amin(xAxis), np.amax(xAxis)), origin = 'lower',
        alpha = 0.8, cmap=greens_alpha, norm=colors.SymLogNorm(linthresh=linThresh*rho_mf1_max,linscale=linScale,vmin=0.,vmax=rho_mf1_max))#vmax=rhoMax))
    putLabels('', r'$x\ \ (\ell)$', r'$\rho \ \ (\frac{1}{\ell^2})$')
    ax = plt.gca()
    ax.set_aspect('auto')

    # Plot mf = 1 rho
    rho_mf1m = (RhoFieldComp[:,:,6]+RhoFieldComp[:,:,7]).real
    rho_mf1m_max = np.amax(rho_mf1m)
    plt.subplot(211)
    plt.imshow((rho_mf1m), extent=(np.amin(yAxis), np.amax(yAxis), np.amin(xAxis), np.amax(xAxis)), origin = 'lower',
        alpha = 0.8, cmap=reds_alpha, norm=colors.SymLogNorm(linthresh=linThresh*rho_mf1m_max,linscale=linScale,vmin=0.,vmax=rho_mf1m_max))#vmax=rhoMax))
    putLabels('', r'$x\ \ (\ell)$', r'$\rho \ \ (\frac{1}{\ell^2})$')
    ax = plt.gca()
    ax.set_aspect('auto')
