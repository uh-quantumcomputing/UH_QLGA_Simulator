...
get_QS = gpuMagic.get_function("getQS_r1_r2_2d")
get_Rho_projected = gpuMagic.get_function("getRho_projected_2d")
...
def make_frame(frame_dir, frame, image_dir, frames, global_vars, find_total_max = False, save_density = False, save_entanglement = False, save_expectation_value = False, exp_range = [0,0], **kwargs):
    ...
    QuantumField_r1_r2_real = np.zeros((xSize, ySize, xSize, ySize), dtype = np.float64)
    ...
    get_QS(gpuQField, gpuQF_r1_r2_real, gpuQF_r1_r2_imag, gpuLattice, block=(blockX,blockY,blockZ), grid=(gridX,gridY,gridZ))
    get_Rho_projected(gpuQF_r1_r2_real, gpuQF_r1_r2_imag, gpuRho_projected_real, gpuLattice, block=(blockX_r1_r2,blockY_r1_r2,blockZ_r1_r2), grid=(gridX_r1_r2,gridY_r1_r2,gridZ_r1_r2))
    ...
    QuantumField_r1_r2_real = drv.from_device(gpuQF_r1_r2_real, QuantumField_r1_r2_real.shape, np.float64)
    ...
    # Density
    plt.subplot(111)
    plt.imshow(((RhoFieldProjected.real).T), extent=(np.amin(xAxis), np.amax(xAxis), np.amin(yAxis), np.amax(yAxis)), origin = 'lower',
        cmap=colorMapRho, norm=colors.SymLogNorm(linthresh=linThresh*yMax,linscale=linScale,vmin=0.,vmax=yMax))
    putLabels(r'$x\ \ (\ell)$', r'$y\ \ (\ell)$', r'$\rho \ \ (\frac{1}{\ell^2})$')
    ax = plt.gca()
    ax.set_aspect('equal')
